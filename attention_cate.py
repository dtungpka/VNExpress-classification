...
def parse_args():
    ...
    parser.add_argument('--classification_task', type=str, default='category',
                        choices=['author', 'category'],
                        help='Classification task: author or category')
...
def get_json_data(folder_path):
    ...
    for article in tqdm(articles, desc="Loading data"):
        with open(os.path.join(folder_path, article), "r", encoding="utf-8") as f:
            ...
            category = data.get("category_code", None)
...
def load_category_definitions(path='category_definitions.json'):
...
def main():
    ...
    if args.experiment_id is None:
        args.experiment_id = f"{args.model_type}_{args.feature_type}_category"
    if args.classification_task == 'category':
        try:
            category_defs = load_category_definitions()
            print(f"Loaded {len(category_defs)} category definitions")
        except:
            print("Warning: Could not load category definitions")
# Create output directory
    output_dir = os.path.join(args.output_dir, f"category_{args.experiment_id}")
    ...
    y = sen_data['category']  # Use category instead of author
    category_counts = y.value_counts()
    categories = category_counts[category_counts >= args.min_articles].index
    sen_data_filtered = sen_data[sen_data['category'].isin(categories)]
    X = sen_data_filtered['tokenized_text']
    y = sen_data_filtered['category']
    print(f"Number of categories with at least {args.min_articles} articles: {len(categories)}")
